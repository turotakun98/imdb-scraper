...
var Log = (function () {
    ...
    function init(path, applicationName, minLogLevel, logInConsole) {
        ...
        function LogMessage(message, logLevel = minLogLevel, functionName = "", fileName = "") {
            if (logLevel.value >= minLogLevel.value) {
                ...
                fs.appendFile(path + filePathName, logMessage + " \r\n", "utf-8", function (err) {
                    if (err) throw err;
                });
    ...
    return {
        createLogger: function ({ path = "./", applicationName = "defaultAppName", minLogLevel = LogLevels.info, logInConsole = true } = {}) {
            if (!instance) {
